...
# enable overwriting Macros in Makefile cmd line
ifdef DGEMM_KC
K_SIZE += -DDGEMM_KC=$(DGEMM_KC)
endif
ifdef DGEMM_MC
K_SIZE += -DDGEMM_MC=$(DGEMM_MC)
endif
ifdef DGEMM_NC
K_SIZE += -DDGEMM_NC=$(DGEMM_NC)
endif
ifdef DGEMM_MR
K_SIZE += -DDGEMM_MR=$(DGEMM_MR)
# define BL_MICRO_KERNEL bl_dgemm_{8}44
K_SIZE += -DBL_MICRO_KERNEL=bl_dgemm_$(DGEMM_MR)44
endif
ifdef DGEMM_NR
K_SIZE += -DDGEMM_NR=$(DGEMM_NR)
endif
C++FLAGS += $(K_SIZE)
CFLAGS += $(K_SIZE)
